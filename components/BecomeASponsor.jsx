import { BecomeSponsorRowData, BecomeSponsorTableHeadData } from "@/constants";
  ...
  return (
    <div className="overflow-x-scroll">
        ...
        <thead className="divide-x divide-y divide-gray-200 border-b font-medium">
            ...
            {BecomeSponsorTableHeadData.map((name,index) => <th key={index} className="px-6 py-4">{name}</th>)}
            </tr>
        </thead>
        <tbody className="divide-y divide-gray-200">
            {BecomeSponsorRowData.map((row, index) => <SponsorTableRow key={index} {...row}/>)}
        </tbody>
        </table>
    </div>
  );
...
function SponsorTableRow({ title, values, icon: Icon, startColumn = 0 }) {
  return (
    <tr className="divide-x divide-gray-200">
      <td className="py-2 px-1 text-center" >{title}</td>
      {values ? (
        values.map((value, index) => (
          ...
          >
            {value === "✓" ? (<p className="text-green text-[20px]">{value}</p>) : value}
          </td>
        ))
      ) : (
